...
sub xq_check {
  ...
  if (assert_file_exists("  Error", "$path/$file")) { 
    ...
    if ($query !~ /doc\([^\)]+\)/) {
      system("echo 'doc(\"voc.xml\")' >$file.prefix"); 
    } else {
      system("echo '' >$file.prefix");
    }
    if ($query =~ /'/) {
      print "  warning: do not use single quotes in your query.\n";
    }
    ...
    system("cat $file.prefix $path/$file | sed -e 's/doc([^)]\\+)/doc(\"\\/home\\/hiemstra\\/Tools\\/dietje-ai\\/courses\\/utwente-ds\\/voc.xml\")/g' > $file");
