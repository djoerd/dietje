...
if (!xq_check("$REPO/$TASK", "assignment3l.xq", 'for $voyage in doc("...")/VOC/voyage[.//master] let $master := $voyage//master let $cnt := count($voyage) group by $master order by $cnt descending return <master name="{$master}" nrofvoyages="{$cnt}"/>')) {
  print "  Ok.\n";
  $points++;
}
else {
...
print "Exercise 3m: (master with most voyages)\n";
if (!xq_check("$REPO/$TASK", "assignment3m.xq", 'let $masters := for $voyage in doc("...")/VOC/voyage[.//master] let $master := $voyage//master let $cnt := count($voyage) group by $master order by $cnt descending return <master name="{$master}" nrofvoyages="{$cnt}"/> return $masters[1]')) {
  print "  Ok.\n";
  $points++;
}
else {
